<template>
    <div>
        <div class="modal fade" id="theaterEditModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">映画館情報</h5>
    ...
    export default {
        ...
        methods: {
            open: function () {
                ...
                $('#theaterEditModal').modal('show');
            },
            close: function(){
                this.user.attributes.name = this.name;
                this.user.attributes.email = this.email;

                $('#theaterEditModal').modal('hide');
            },
            updateData: function () {
                if (this.newPassword === this.confirmPassword){
                    axios.patch('/api/v1/users/' + this.innerSearchText.id,{
                        'name': this.name,
                        'email': this.email,
            ...
            deleteData: function () {
                axios.delete('/api/v1/users/' + this.innerSearchText.id)
                    .then(() => {
